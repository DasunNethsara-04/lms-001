...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    if ($op == 1) {
        ...
        if ($_POST['email'] != "") {
            ...
            $stmt1 = $conn->prepare("UPDATE teacher_tbl SET email=? WHERE email=?");
            ...
            if ($stmt1->affected_rows > 0) {
                ...
                header("Location: ../Pages/Teacher/settings.php?success=$success_message");
                exit();
            } else {
                // updation error
                $error_message = "Data Updating faild";
                $error_message = htmlspecialchars($error_message, ENT_QUOTES, 'UTF-8');
                header("Location: ../Pages/Teacher/settings.php?error=$error_message");
                exit();
            }
        } else {
            ...
            header("Location: ../Pages/Teacher/settings.php?warning=$warning_message");
            exit();
        }
        ...
        if ($_POST['old_pwd'] != "" && $_POST['new_pwd'] != "" && $_POST['confirmed_new_pwd'] != "") {
            ...
            $status = 1;
            $stmt1 = $conn->prepare("SELECT password FROM teacher_tbl WHERE email= ? AND status= ?");
            ...
            if (password_verify($old_pwd, $o_pwd)) {
                if ($new_pwd == $confirmed_pwd) {
                    ...
                    $stmt2 = $conn->prepare("UPDATE teacher_tbl SET password=? WHERE email=?");
